...
class ExampleInfoGet200ResponseInner(BaseModel):
    """
    ExampleInfoGet200ResponseInner
    """ # noqa: E501
    content_type: StrictStr = Field(alias="contentType")
    data: ExtInfoExampleInfoExampleData
    __properties: ClassVar[List[str]] = ["contentType", "data"]

    @field_validator('content_type')
    def content_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['image/jpeg']):
            raise ValueError("must be one of enum values ('image/jpeg')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "data": ExtInfoExampleInfoExampleData.from_dict(obj["data"]) if obj.get("data") is not None else None
        })
        return _obj
